...
const DocumentCard = ({ document, onDelete }) => {
  ...
  const handleDownload = async () => {
    ...
    setLoading(true);
    const { data: url } = await customerJourneyService?.getDocumentUrl(document?.filePath);
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    const { error } = await customerJourneyService?.deleteDocument(document?.id, document?.filePath);
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-4 hover:shadow-md transition-all duration-300">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        {document?.status !== 'required' && document?.filePath && (
          <>
            <Button 
              variant="outline" 
              size="sm"
        ...
        {(document?.status === 'rejected' || document?.status === 'required') && (
          <Button 
            variant="default" 
            size="sm"
            className="flex-1"
            ...
            disabled={loading}
          >
            {document?.status === 'required' ? 'Upload' : 'Re-upload'}
          </Button>
        )}
      </div>
    </div>
